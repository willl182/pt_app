...
| **Tabla ANOVA** | - | `variance_components` | `tableOutput` | Componentes de varianza ($s_s, s_w$) y criterios. |
...
| **Tabla $u_{hom}$** | - | `u_hom_table` | `dataTableOutput` | Incertidumbre por homogeneidad. |
| **Tabla $u_{stab}$** | - | `u_stab_table` | `dataTableOutput` | Incertidumbre por estabilidad. |
...
2.  **`homogeneity_run()`**: Depende de los selectores de analito/nivel y del disparador (*trigger*). Llama a `compute_homogeneity_metrics()`.
3.  **`stability_run()`**: Depende de que `homogeneity_run()` se haya completado exitosamente. Evalúa la estabilidad comparando medias.
4.  **`algoA_results_cache()`**: Almacena los resultados del Algoritmo A para evitar recálculos innecesarios. Se limpia si los datos de entrada cambian.
...
3.  Calcula componentes de varianza ($s_s, s_w$) mediante ANOVA.
4.  Evalúa criterios:
    *   Criterio Crítico: $s_s \leq 0.3 \times \sigma_{pt}$
    *   Criterio Expandido: $s_s \leq \sqrt{(0.3\sigma_{pt})^2 \times 1.88 + s_w^2 \times 1.01}$
5.  Retorna una lista con resultados, conclusiones y clases CSS para la UI.
...
### Rango Intercuartil Normalizado (nIQR)
...
sequenceDiagram
    ...
    participant Cache
    participant AlgoA
    User->>UI: Clic en "Ejecutar Algoritmo A"
