# Módulo: Informe Global (Global Report)
...
### Entradas (Inputs)
| Elemento UI | ID del Input | Tipo | Descripción |
|-------------|--------------|------|-------------|
| Selector Analito | `global_report_pollutant` | `selectInput` | Filtra por contaminante (CO, NO, NO2, etc.) |
| Selector Esquema | `global_report_n_lab` | `selectInput` | Filtra por esquema PT (n_lab) |
| Selector Nivel | `global_report_level` | `selectInput` | Filtra por nivel de concentración |
| Filtro de Método | `global_method_filter` | `selectInput` | Filtra por método de valor asignado (1, 2a, 2b, 3) |

### Salidas (Outputs)
| Elemento UI | ID del Output | Tipo | Descripción |
|-------------|---------------|------|-------------|
| Info tamaño PT | `global_report_pt_size_info` | `uiOutput` | Muestra número de participantes y referencia |
| Tabla x_pt | `global_xpt_summary_table` | `dataTableOutput` | Resumen de valores asignados y parámetros |
| Tabla Evaluaciones | `global_evaluation_summary_table` | `dataTableOutput` | Conteo de resultados por categoría (Satisfactorio, etc.) |
| Tabla Resumen DT | `global_summary_dt` | `dataTableOutput` | Tabla general de parámetros estadísticos |
| Tabla Overview DT | `global_overview_dt` | `dataTableOutput` | Tabla detallada de puntajes por participante |
| Heatmaps z/z'/ζ/En | `global_heatmap_*_ref` | `plotlyOutput` | Mapas de calor interactivos por método y tipo de puntaje |

---

## Arquitectura y Flujo de Datos

El módulo utiliza un pipeline de agregación que recolecta datos del caché de resultados de puntajes.

### Pipeline de Agregación
1.  **Activación:** El reactivo `global_report_data()` se dispara cuando cambian los puntajes calculados.
...
- Tasa de aprobación (Pass Rate %):
  $$\text{Pass Rate} = \frac{\text{Conteo}(|z| \le 2)}{\text{Total Participantes}} \times 100$$
...
## Visualizaciones: Mapas de Calor (Heatmaps)
...
### Paleta de Colores (Traffic Light System)
...
Cada una incluye sus propios parámetros ($x_{pt}, \sigma_{pt}, u(x_{pt})$) y un grid de 2x2 con los heatmaps para los cuatro tipos de puntajes.
...
- **Generación Dinámica**: Los heatmaps se renderizan solo cuando la pestaña correspondiente es visible.
