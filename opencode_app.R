...
")
  ...
  output$results_histogram <- renderPlotly({
    ...
    req(plot_data)
    ggplotly(ggplot(plot_data, aes(x = result)) +
  ...
  output$results_boxplot <- renderPlotly({
    plot_data <- plot_data_long()
    req(plot_data)
    ggplotly(ggplot(plot_data, aes(x = "", y = result)) +
...
")
  ...
  render_global_score_heatmap <- function(output_id, combo_key, score_col, eval_col, palette, title_prefix) {
    output[[output_id]] <- renderPlotly({
      ...
      if (nrow(filtered) == 0) {
        return(ggplotly(ggplot() + theme_void() + labs(title = paste(title_prefix, "- sin datos disponibles"))))
      }
      ...
      ggplotly(ggplot(plot_data, aes(x = run_label, y = participant_id, fill = evaluation)) +
  ...
  render_global_classification_heatmap <- function(output_id, combo_key, code_col, label_col, title_prefix) {
    output[[output_id]] <- renderPlotly({
      ...
      ggplotly(ggplot(plot_data, aes(x = run_label, y = participant_id, fill = fill_code)) +
        geom_tile(color = "white") +
  ...
  output$scores_participant_tabs <- renderUI({
    ...
    tab_panels <- lapply(participants, function(pid) {
      ...
      output[[plot_id]] <- renderPlotly({
        ...
        p_values <- ggplotly(ggplot(plot_df, aes(x = factor(level, levels = sort(unique(level))))) +
        ...
        p_z <- ggplotly(ggplot(participant_df, aes(x = level_factor, y = z_score, group = combination, color = combination)) +
        ...
        p_zeta <- ggplotly(ggplot(participant_df, aes(x = level_factor, y = zeta_score, group = combination, color = combination)) +
          geom_hline(yintercept = c(-3, 3), linetype = "dashed", color = "#C0392B") +
          geom_hline(yintercept = c(-2, 2), linetype = "dashed", color = "#E67E22") +
          geom_hline(yintercept = 0, color = "grey50") +
          geom_line(position = position_dodge(width = 0.3)) +
        ...
        p_en <- ggplotly(ggplot(participant_df, aes(x = level_factor, y = En_score, group = combination, color = combination)) +
        ...
        subplot(p_values, p_z, p_zeta, p_en, nrows = 2, titleX = TRUE, titleY = TRUE)
      })
  ...
  output$algoA_histogram <- renderPlotly({
    ...
    ggplotly(ggplot(res$input_data, aes(x = Resultado)) +
    ...
    lapply(pollutants, function(p) {
      local({
        ...
        output[[paste0("pt_plot_", pollutant_name)]] <- renderPlotly({
          ...
          ggplotly(ggplot(data, aes(x = participant_id, y = mean_value, fill = sample_group)) +
        ...
        output[[paste0("pt_histogram_", pollutant_name)]] <- renderPlotly({
          ...
          ggplotly(ggplot(participants_data, aes(x = mean_value)) +
        ...
        output[[paste0("pt_boxplot_", pollutant_name)]] <- renderPlotly({
          ...
          ggplotly(ggplot(participants_data, aes(x = "", y = mean_value)) +
        ...
        output[[paste0("pt_runchart_", pollutant_name)]] <- renderPlotly({
          ...
          ggplotly(ggplot(participants_data, aes(x = sample_group, y = mean_value, group = 1)) +
            geom_point() +
            geom_line() +
