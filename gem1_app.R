...
server <- function(input, output, session) {
  ...
  purrr::iwalk(global_combo_specs, function(spec, combo_key) {
    ...
    }, striped = TRUE, spacing = "l", rownames = FALSE) # Removed digits argument as it's not standard for renderTable
  ...
  output$scores_participant_tabs <- renderUI({
    ...
    tab_panels <- lapply(participants, function(pid) {
      ...
      output[[plot_id]] <- renderPlotly({
        ...
        subplot(ggplotly(p_values), ggplotly(p_z), ggplotly(p_zeta), ggplotly(p_en), nrows = 2, shareX = TRUE, shareY = FALSE)
      })
