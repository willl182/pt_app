...
def create_algorithm_a_sheet(wb, summary_data):
    ...
    Algorithm A (ISO 13528:2022 Annex C.3):
    1. Initialize: x* = median, s* = 1.483 × MAD
    2. delta = 1.5 × s*
    3. xi* = clamp(xi, x* - delta, x* + delta)
    4. x*_new = mean(xi*)
    5. s*_new = 1.134 × STDEV.S(xi*)
    ...
    ws['A2'] = "ISO 13528:2022 Annex C.3 - Iterative robust estimation by winsorization"
    ws.merge_cells('A2:I2')

    ws['A3'] = "Note: Shows iteration 0 and iteration 1 formulas using winsorization."
    ws['A3'].font = Font(italic=True)
    ws.merge_cells('A3:I3')

    ws['A5'] = "ALGORITHM PARAMETERS"
    ws['A5'].font = Font(bold=True)

    params = [
        ('p (observations)', n, None),
    ...
    for row_idx, (label, value, note) in enumerate(params, start=6):
        ws.cell(row=row_idx, column=1, value=label)
        ws.cell(row=row_idx, column=2, value=value)
        ws.cell(row=row_idx, column=2).fill = INPUT_FILL
        if note:
            ws.cell(row=row_idx, column=3, value=f'← {note}')
    ...
    for i, v in enumerate(values, start=12):
        ws.cell(row=i, column=1, value=i - 11)
    ...
    ws.cell(row=init_row + 1, column=2, value=f'=MEDIAN(B12:B{data_end})')
    ws.cell(row=init_row + 1, column=2).font = FORMULA_FONT
    ws.cell(row=init_row + 1, column=2).fill = RESULT_FILL
    x_star_cell = f'B{init_row + 1}'

    for i in range(12, data_end + 1):
    ...
    ws.cell(row=init_row + 2, column=2, value=f'=MEDIAN(C12:C{data_end})')
    ws.cell(row=init_row + 2, column=2).font = FORMULA_FONT

    ws.cell(row=init_row + 3, column=1, value="s*_0 = 1.483 × MAD")
    ws.cell(row=init_row + 3, column=2, value=f'=$B$7*B{init_row + 2}')
    ws.cell(row=init_row + 3, column=2).font = FORMULA_FONT
    ws.cell(row=init_row + 3, column=2).fill = RESULT_FILL
    s_star_cell = f'B{init_row + 3}'

    ws.cell(row=init_row + 4, column=1, value="delta_0 = 1.5 × s*_0")
    ws.cell(row=init_row + 4, column=2, value=f'=$B$8*{s_star_cell}')
    ws.cell(row=init_row + 4, column=2).font = FORMULA_FONT
    ws.cell(row=init_row + 4, column=2).fill = RESULT_FILL
    delta_cell = f'B{init_row + 4}'

    ws.cell(row=init_row + 5, column=1, value="Lower bound = x*_0 - delta_0")
    ws.cell(row=init_row + 5, column=2, value=f'={x_star_cell}-{delta_cell}')
    ws.cell(row=init_row + 5, column=2).font = FORMULA_FONT
    ws.cell(row=init_row + 5, column=2).fill = RESULT_FILL
    lower_cell = f'B{init_row + 5}'

    ws.cell(row=init_row + 6, column=1, value="Upper bound = x*_0 + delta_0")
    ws.cell(row=init_row + 6, column=2, value=f'={x_star_cell}+{delta_cell}')
    ws.cell(row=init_row + 6, column=2).font = FORMULA_FONT
    ws.cell(row=init_row + 6, column=2).fill = RESULT_FILL
    upper_cell = f'B{init_row + 6}'

    iter_row = init_row + 9
    ws.cell(row=iter_row, column=1, value="ITERATION 1 DETAIL - WINSORIZATION")
    ws.cell(row=iter_row, column=1).font = Font(bold=True)

    headers = ['i', 'xi', 'Lower bound', 'Upper bound', 'xi* (winsorized)', '(xi* - x*_1)^2']
    for col, h in enumerate(headers, 1):
        ws.cell(row=iter_row + 1, column=col, value=h)
    style_header_row(ws, iter_row + 1, len(headers))

    for row_idx, value_row in enumerate(range(12, data_end + 1), start=iter_row + 2):
        ws.cell(row=row_idx, column=1, value=row_idx - iter_row - 1)
        ws.cell(row=row_idx, column=2, value=f'=B{value_row}')
        ws.cell(row=row_idx, column=2).font = FORMULA_FONT
        ws.cell(row=row_idx, column=3, value=f'={lower_cell}')
        ws.cell(row=row_idx, column=3).font = FORMULA_FONT
        ws.cell(row=row_idx, column=4, value=f'={upper_cell}')
        ws.cell(row=row_idx, column=4).font = FORMULA_FONT
        ws.cell(row=row_idx, column=5, value=f'=MAX(MIN(B{row_idx},{upper_cell}),{lower_cell})')
        ws.cell(row=row_idx, column=5).font = FORMULA_FONT

    iter_end = iter_row + 1 + n

    ws.cell(row=iter_end + 1, column=1, value="x*_1 = mean(xi*)")
    ws.cell(row=iter_end + 1, column=2, value=f'=AVERAGE(E{iter_row + 2}:E{iter_end})')
    ws.cell(row=iter_end + 1, column=2).font = FORMULA_FONT
    ws.cell(row=iter_end + 1, column=2).fill = RESULT_FILL
    x_new_cell = f'B{iter_end + 1}'

    for row_idx in range(iter_row + 2, iter_end + 1):
        ws.cell(row=row_idx, column=6, value=f'=(E{row_idx}-{x_new_cell})^2')
        ws.cell(row=row_idx, column=6).font = FORMULA_FONT

    ws.cell(row=iter_end + 2, column=1, value="s*_1 = 1.134 × STDEV.S(xi*)")
    ws.cell(row=iter_end + 2, column=2, value=f'=$B$9*STDEV.S(E{iter_row + 2}:E{iter_end})')
    ws.cell(row=iter_end + 2, column=2).font = FORMULA_FONT
    ws.cell(row=iter_end + 2, column=2).fill = RESULT_FILL
    s_new_cell = f'B{iter_end + 2}'

    ws.cell(row=iter_end + 4, column=1, value="Δx* = |x*_1 - x*_0|")
    ws.cell(row=iter_end + 4, column=2, value=f'=ABS({x_new_cell}-{x_star_cell})')
    ws.cell(row=iter_end + 4, column=2).font = FORMULA_FONT

    ws.cell(row=iter_end + 5, column=1, value="Δs* = |s*_1 - s*_0|")
    ws.cell(row=iter_end + 5, column=2, value=f'=ABS({s_new_cell}-{s_star_cell})')
    ws.cell(row=iter_end + 5, column=2).font = FORMULA_FONT

    ws.cell(row=iter_end + 6, column=1, value="Converged? (max(Δx*, Δs*) < 1e-6)")
    ws.cell(row=iter_end + 6, column=2, value=f'=IF(MAX(B{iter_end + 4},B{iter_end + 5})<1E-6,"YES","NO")')
    ws.cell(row=iter_end + 6, column=2).font = FORMULA_FONT
    ws.cell(row=iter_end + 6, column=2).fill = RESULT_FILL

    summary_row = iter_end + 9
    ws.cell(row=summary_row, column=1, value="ITERATION SUMMARY")
    ws.cell(row=summary_row, column=1).font = Font(bold=True)

    summary_headers = ['Iteration', 'x*', 's*', 'delta', 'Converged']
    for col, h in enumerate(summary_headers, 1):
        ws.cell(row=summary_row + 1, column=col, value=h)
    style_header_row(ws, summary_row + 1, len(summary_headers))

    ws.cell(row=summary_row + 2, column=1, value=0)
    ws.cell(row=summary_row + 2, column=2, value=f'={x_star_cell}')
    ws.cell(row=summary_row + 2, column=3, value=f'={s_star_cell}')
    ws.cell(row=summary_row + 2, column=4, value=f'={delta_cell}')
    ws.cell(row=summary_row + 2, column=5, value='-')

    ws.cell(row=summary_row + 3, column=1, value=1)
    ws.cell(row=summary_row + 3, column=2, value=f'={x_new_cell}')
    ws.cell(row=summary_row + 3, column=3, value=f'={s_new_cell}')
    ws.cell(row=summary_row + 3, column=4, value=f'=1.5*{s_new_cell}')
    ws.cell(row=summary_row + 3, column=5, value=f'=B{iter_end + 6}')
    for col in range(2, 6):
        ws.cell(row=summary_row + 3, column=col).font = FORMULA_FONT
        ws.cell(row=summary_row + 3, column=col).fill = RESULT_FILL

    final_row = summary_row + 6
    ws.cell(row=final_row, column=1, value="FINAL RESULTS (after displayed iteration)")
    ws.cell(row=final_row, column=1).font = Font(bold=True)

    ws.cell(row=final_row + 1, column=1, value="x* (Robust Mean / Assigned Value)")
    ws.cell(row=final_row + 1, column=2, value=f'={x_new_cell}')
    ws.cell(row=final_row + 1, column=2).font = FORMULA_FONT
    ws.cell(row=final_row + 1, column=2).fill = RESULT_FILL

    ws.cell(row=final_row + 2, column=1, value="s* (Robust SD)")
    ws.cell(row=final_row + 2, column=2, value=f'={s_new_cell}')
    ws.cell(row=final_row + 2, column=2).font = FORMULA_FONT
    ws.cell(row=final_row + 2, column=2).fill = RESULT_FILL

    ws.cell(row=final_row + 3, column=1, value="u(x_pt) = 1.25 × s* / sqrt(p)")
    ws.cell(row=final_row + 3, column=2, value=f'=1.25*B{final_row + 2}/SQRT($B$6)')
    ws.cell(row=final_row + 3, column=2).font = FORMULA_FONT
    ws.cell(row=final_row + 3, column=2).fill = RESULT_FILL

    ref_row = final_row + 6
    ws.cell(row=ref_row, column=1, value="FORMULA REFERENCE (ISO 13528:2022 Annex C.3 - Winsorization Method)")
    ws.cell(row=ref_row, column=1).font = Font(bold=True)

    formulas = [
        ("Initial x*", "x*_0 = median(xi)"),
        ("Initial s*", "s*_0 = 1.483 × MAD(xi)"),
        ("Winsorization bound", "delta = 1.5 × s*"),
        ("Winsorized value", "xi* = clamp(xi, x* - delta, x* + delta)"),
        ("Updated x*", "x*_new = mean(xi*)"),
        ("Updated s*", "s*_new = 1.134 × STDEV.S(xi*)"),
        ("Convergence", "Stop when max(|Δx*|, |Δs*|) < 1e-6"),
    ]
