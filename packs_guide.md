...
algorithm\_A \<- function(x, max\_iter \= 100\) {
  ...
  x\_star \<- stats::median(x)

  s\_star \<- stats::mad(x, constant \= 1.4826)
  ...
  for (i in 1:max\_iter) {
    ...
    x\_star \<- base::mean(x\_prime)

    s\_star \<- 1.134 \* stats::sd(x\_prime)
...
fit\_simple\_model \<- function(formula, data) {
...
\#' @param max\_iter An integer specifying the maximum number of iterations.
...
\#' sample\_data \<- c(10.1, 10.2, 10.5, 9.9, 10.3, 25.0)

\#' algorithm\_A(sample\_data)
...
algorithm\_A \<- function(x, max\_iter \= 100\) {
  ...
  for (i in 1:max\_iter) {
    ...
    if (signif(x\_star, 3\) \== signif(x\_star\_prev, 3\) && signif(s\_star, 3\) \== signif(s\_star\_prev, 3)) {
      ...
      return(list(robust\_mean \= x\_star, robust\_sd \= s\_star, iterations \= i \- 1))
  ...
  return(list(robust\_mean \= x\_star, robust\_sd \= s\_star, iterations \= max\_iter))
...
\#' fit\_simple\_model(dist \~ speed, data \= cars)
...
The `lm()` function (for **l**inear **m**odel) is the workhorse for fitting linear models in R. When we call `lm()` with a formula and a dataset, it returns a *model fit object* that contains all the details of the fitted model.
