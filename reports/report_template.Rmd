...
safe_rename_by_position <- function(df, labels) {
  if (!is.data.frame(df) || nrow(df) == 0) {
    return(df)
  }
  names(df)[seq_len(min(ncol(df), length(labels)))] <- labels[seq_len(min(ncol(df), length(labels)))]
  df
}

selected_summary_data <- function() {
  df <- safe_param_df(params$summary_data)
  if (nrow(df) == 0) {
    return(df)
  }
  if ("n_lab" %in% names(df) && !is.null(params$n_lab)) {
    df <- df %>% filter(n_lab == params$n_lab)
  }
  if ("level" %in% names(df) && !is.null(params$level)) {
    df <- df %>% filter(level == params$level)
  }
  df
}

round_summary_data <- function() {
  df <- safe_param_df(params$summary_data)
  if (nrow(df) == 0) {
    return(df)
  }
...
participant_count <- function(df) {
  if (!is_nonempty_df(df) || !"participant_id" %in% names(df)) {
    return(0L)
  }
  length(unique(df$participant_id[df$participant_id != "ref" & !is.na(df$participant_id)]))
}
...
**Profesional Proyectos:** `r params$projects_pro`  
**Profesional Infraestructura:** `r params$infrastructure_pro`  
...
En esta ronda del EA participaron `r participant_count(selected_summary_data())` laboratorios. A continuación, se lista a los participantes (mediante código asignado).

```{r participants_table, echo=FALSE, results='asis'}
final_part_df <- safe_param_df(params$equipos_data)

if (!is_nonempty_df(final_part_df)) {
  final_part_df <- safe_param_df(params$participants_data)
}

if (is_nonempty_df(final_part_df)) {
  final_part_df <- unique(final_part_df)
  final_part_df <- final_part_df %>%
    select(any_of(c(
      "Codigo_Lab",
      "Analizador_SO2",
      "Analizador_CO",
  ...
  display_names <- c(
    Codigo_Lab = "Código Lab",
    Analizador_SO2 = "Analizador SO₂ (Marca/Modelo)",
    Analizador_CO = "Analizador CO (Marca/Modelo)",
    Analizador_O3 = "Analizador O₃ (Marca/Modelo)",
    Analizador_NO_NO2 = "Analizador NO/NO₂ (Marca/Modelo)"
  )
  matched_names <- names(final_part_df)
  matched_names[matched_names %in% names(display_names)] <- display_names[matched_names[matched_names %in% names(display_names)]]
  names(final_part_df) <- matched_names

  print(
    kable(
      final_part_df,
      caption = "Tabla 2. Participantes del Ensayo de Aptitud"
    )
  )
} else {
  cat("No hay datos de participantes disponibles para la ronda seleccionada.")
}
```
...
summary_selected <- round_summary_data()

if (is_nonempty_df(summary_selected)) {
  levels_source <- summary_selected
  if ("participant_id" %in% names(levels_source) && any(levels_source$participant_id == "ref", na.rm = TRUE)) {
    levels_source <- levels_source %>% filter(participant_id == "ref")
  }
  if (!"run" %in% names(levels_source)) {
    levels_source <- levels_source %>% mutate(run = level)
  }
  levels_data <- levels_source %>%
    select(run, pollutant, mean_value) %>%
    group_by(run, pollutant) %>%
    summarise(mean_value = mean(mean_value, na.rm = TRUE), .groups = "drop") %>%
    pivot_wider(names_from = pollutant, values_from = mean_value) %>%
    mutate(run_order = readr::parse_number(as.character(run))) %>%
    arrange(run_order, run) %>%
    select(-run_order)
...
if (!is.null(params$method) && as.character(params$method) == "1") {
  ...
  cat("La desviación estándar para la evaluación de la aptitud ($\\sigma_{pt}$) se obtiene de la evaluación de homogeneidad para cada contaminante y nivel.")
...
if (method == "2a") {
  ...
  cat("Se evaluó la compatibilidad metrológica para el criterio de expertos, usando el valor de referencia como $x_{pt}$ y la desviación para evaluación de aptitud calculada con la ecuación operativa por contaminante.")
} else {
...
if (!is.null(params$metrological_compatibility) && is.data.frame(params$metrological_compatibility) && nrow(params$metrological_compatibility) > 0) {
  ...
  if (method == "2a") {
    ...
    mc_df <- mc_df %>% select(pollutant, n_lab, level, x_pt_ref, x_pt_4, sigma_pt_4, Diff_Ref_4)
    colnames(mc_df) <- c("Contaminante", "N_Lab", "Nivel", "Valor Ref", "Valor Expertos 4", "sigma_pt Expertos", "Dif (Ref - 4)")
  } else {
    # Fallback: Show all
...
if (is_nonempty_df(params$score_summary)) {
...
A continuación se presenta el resumen de los valores asignados ($x_{pt}$), incertidumbres ($u(x_{pt})$) y desviaciones estándar para la evaluación de la aptitud ($\sigma_{pt}$) para cada contaminante y nivel. La tabla incluye el método seleccionado y, cuando corresponde, el criterio de expertos con $x_{pt}$ tomado de la referencia.

```{r annex_a_table, echo=FALSE}
if (is_nonempty_df(params$xpt_summary)) {
  
  xpt_df <- params$xpt_summary
  
  # Ensure proper column names for display
  xpt_df <- safe_rename_by_position(
    xpt_df,
    c("Contaminante", "Nivel", "Método", "Valor Asignado (x_pt)", "Incertidumbre u(x_pt)", "u_ref", "Desviación (sigma_pt)")
  )
...
if (is_nonempty_df(params$homogeneity_summary)) {
  
  hom_df <- params$homogeneity_summary
  
  # Proper column names for display
  hom_df <- safe_rename_by_position(
    hom_df,
    c("Contaminante", "Nivel", "Items", "Réplicas", "sigma_pt", "u(x_pt)", "ss", "sw", "Criterio c", "Criterio c_exp", "Cumple", "Cumple c_exp", "Conclusión")
  )
...
if (is_nonempty_df(params$stability_summary)) {
  
  stab_df <- params$stability_summary
  
  # Proper column names for display
  stab_df <- safe_rename_by_position(
    stab_df,
    c("Contaminante", "Nivel", "Items", "Réplicas", "sigma_pt", "u(x_pt)", "Diferencia", "Criterio c", "Criterio c_exp", "Cumple", "Cumple c_exp", "Conclusión")
  )
...
if (!is.null(params$participant_data) && length(params$participant_data) > 0) {
  for (pid in names(params$participant_data)) {
    ...
    disp_table <- safe_rename_by_position(
      disp_table,
      c("Contaminante", "Nivel", "Método", "Resultado", "Incertidumbre", "Valor Asignado", "Incertidumbre VA", "sigma_pt", "Score", "Evaluación")
    )
