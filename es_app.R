...
#
# Esta aplicación implementa los procedimientos de las normas ISO 17043 e ISO 13528 
# en una interfaz web interactiva usando Shiny.
#
# ===================================================================
...
server <- function(input, output, session) {
  ...
  pt_prep_data <- reactive({
    ...
    # Almacenar datos crudos en un valor reactivo para el cálculo de sigma_pt_1
    ...
    # Agregar los datos crudos para obtener un valor medio único por participante/nivel
    raw_data %>%
  ...
  # --- Auxiliares de cálculo compartidos ---
  ...
  compute_homogeneity_metrics <- function(target_pollutant, target_level) {
    ...
    # Criterio c expandido (según ISO 13528)
  ...
  compute_scores_metrics <- function(summary_df, target_pollutant, target_n_lab, target_level, sigma_pt, u_xpt, k, m = NULL) {
    ...
    final_scores <- participant_data %>%
      ...
      # Evaluación de puntajes según ISO 13528
      mutate(
  ...
  run_algorithm_a <- function(values, ids, max_iter = 50) {
    ...
    # Bucle de iteración del Algoritmo A
  ...
  # R3: Ejecución de Homogeneidad (Habilitado después de usar el botón ejecutar)
  ...
  # R3.5: Ejecución de Homogeneidad para Datos de estabilidad (Habilitado después de usar el botón ejecutar)
  ...
  # R4: Ejecución de Estabilidad (Habilitado después de usar el botón ejecutar)
  stability_run <- reactive({
    req(analysis_trigger())
    hom_results <- homogeneity_run()
    stab_hom_results <- homogeneity_run_stability()

    # Verificar errores de los reactivos superiores
    if (!is.null(hom_results$error)) {
      return(list(error = hom_results$error))
    }
    ...
    # Usar sigma_pt de la ejecución primaria de homogeneidad
    sigma_pt <- hom_results$sigma_pt
    stab_criterion_value <- 0.3 * sigma_pt

    # Formato dinámico para lugares decimales
    fmt <- "%.5f"
...
Criterio de estabilidad (0.3 * sigma_pt):", fmt),
    ...
    # Para la prueba t, necesitamos los resultados brutos de ambos conjuntos de datos para el nivel seleccionado
  ...
  u_hom_data <- reactive({
    ...
    for (i in seq_len(nrow(combos))) {
      ...
      # Usamos tryCatch para evitar fallos si una combinación falla
      res <- tryCatch({
...
")
  ...
  # Expresión reactiva para datos de graficación
  ...
  # Salida: Tabla de Estadísticas Robustas
  ...
  # Salida: Resumen de Estadísticas Robustas
  ...
  # Output: Stability Summary (PT Preparation Module)
...
")
  ...
  # Salida: Tabla resumen de estadísticas de detalles
  ...
  # --- Salidas para la Pestaña de Análisis de Datos de Estabilidad ---
  ...
  # Salida: Tabla resumen de estadísticas de detalles para Datos de estabilidad
...
#    }
  ...
  metrological_compatibility_data <- eventReactive(input$run_metrological_compatibility, {
    ...
    for(i in seq_len(nrow(final_df))) {
      ...
      # Usamos input$report_k para consistencia con otros reportes.
  ...
  # Reactive data for PT Preparation Module
  ...
  # Reactive: Assigned Value Summary (PT Preparation Module)
  ...
  # Reactive: Homogeneity Summary (PT Preparation Module)
  ...
  # Reactivos de resumen de puntajes
  ...
  # Salida: Resumen de Puntajes PT (Módulo de Preparación PT)
  ...
  # Gestor de Descarga de Informe Individual
