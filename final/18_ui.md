# 18. UI Styling & CSS Architecture

| Property | Value |
|----------|-------|
| **Document Type** | UI/CSS Reference |
| **Primary File** | `www/appR.css` (1458 lines) |
| **Framework** | CSS3 with Custom Properties |
| **Design System** | Custom PT Theme (Primary: #FDB913) |
| **Related Docs** | `15_architecture.md`, `16_customization.md`, `README.md` |

---

## 1. Overview

The PT application uses a custom CSS theme (`www/appR.css`) that provides a modern, accessible, and professional appearance. The stylesheet is organized into logical sections and uses CSS Custom Properties (variables) for consistent theming and easy customization.

### Key Features
- **Modern Design**: Clean aesthetics with subtle shadows and gradients
- **Consistent Theming**: CSS variables for colors, spacing, and typography
- **Responsive Layout**: Mobile-friendly breakpoints
- **Accessibility**: High contrast mode, reduced motion, and keyboard focus support
- **Performance**: Optimized transitions and minimal repaints
- **shadcn-Inspired Components**: Modern card, alert, and badge components

### File Location
```
pt_app/
└── www/
    └── appR.css    # Main stylesheet (1458 lines)
```

### Loading in Shiny
The CSS is loaded automatically when placed in `www/`. It's included in the app via:
```r
# In app.R UI definition
fluidPage(
  theme = bs_theme(...),  # bslib base theme
  includeCSS("www/appR.css"),  # Custom overrides
  ...
)
```

> **Note:** Application screenshots in this documentation should be updated to reflect the current UI design with the enhanced header, shadcn-inspired components, and modern footer. Current screenshots may show the previous design.

---

## 2. CSS Architecture

### 2.1 Section Structure

| Section | Lines | Purpose |
|---------|-------|---------|
| **CSS Variables** | 1-61 | Custom properties for theming |
| **Base Styles** | 63-83 | Body, container defaults |
| **Typography** | 84-148 | Headings, paragraphs, links |
| **Horizontal Rules** | 149-158 | Decorative separators |
| **Cards & Panels** | 159-192 | Well, card, panel styling |
| **Sidebar** | 193-210 | Navigation sidebar |
| **Navigation Tabs** | 211-288 | Tabs, pills, navlist panels |
| **Form Controls** | 289-417 | Inputs, selects, sliders, file upload |
| **Buttons** | 418-491 | Action buttons, button variants |
| **Tables** | 492-582 | DataTables, table styling |
| **Alerts & Notifications** | 583-621 | Shiny notifications, alerts |
| **Progress Bars** | 622-637 | Progress indicators |
| **Score Badges** | 638-670 | PT score status indicators |
| **Plots** | 671-682 | Shiny plots, Plotly containers |
| **Modals** | 683-718 | Modal dialogs |
| **Tooltips** | 719-728 | Tooltip styling |
| **Responsive Design** | 729-754 | Mobile breakpoints |
| **Print Styles** | 755-778 | Print-friendly overrides |
| **Accessibility** | 779-827 | Focus, screen readers, contrast |
| **Header with Logo** | 828-902 | Enhanced app header with logo container |
| **shadcn-Inspired Cards** | 903-960 | Modern card component styling |
| **shadcn-Inspired Alerts** | 961-1021 | Alert variants (info, success, warning, error) |
| **shadcn-Inspired Badges** | 1022-1075 | Badge variants including score-specific |
| **Enhanced Upload Components** | 1076-1159 | Upload grid, styled file inputs |
| **Enhanced DataTables** | 1160-1216 | Modern table with sticky headers |
| **Modern Footer** | 1217-1280 | Three-column footer with sections |
| **Utility Components** | 1281-1354 | Separators, spinners, nav icons |
| **Value Boxes/KPI** | 1355-1391 | KPI card components |
| **Footer** | 1392-1404 | Application footer |
| **Scrollbars** | 1405-1433 | Custom scrollbar styling |
| **RHandsontable** | 1434-1458 | Spreadsheet component styling |

---

## 3. CSS Variables (Custom Properties)

The theme uses CSS Custom Properties defined in `:root` for consistent styling across all components.

### 3.1 Color Palette

```css
:root {
  /* Primary Colors - Yellow Brand */
  --pt-primary: #FDB913;        /* Main brand color */
  --pt-primary-light: #FFD54F;  /* Hover states */
  --pt-primary-dark: #E5A610;   /* Active states */
  --pt-primary-subtle: #F5F5F0; /* Backgrounds */
  
  /* Neutral Colors - Gray Palette */
  --pt-bg: #E8EAED;             /* Page background */
  --pt-bg-card: #F5F6F7;        /* Card backgrounds */
  --pt-fg: #1F2937;             /* Primary text */
  --pt-fg-muted: #6B7280;       /* Secondary text */
  --pt-border: #D1D5DB;         /* Border color */
  --pt-border-focus: #FDB913;   /* Focus border */
  
  /* Secondary & Accent */
  --pt-secondary: #111827;      /* Dark text/headings */
  --pt-success: #38A169;        /* Success states */
  --pt-warning: #ECC94B;        /* Warning states */
  --pt-danger: #E53E3E;         /* Error states */
  --pt-info: #3182CE;           /* Info states */
  
  /* Score Status Colors (ISO 13528) */
  --pt-satisfactory: #00B050;   /* |z| ≤ 2.0 */
  --pt-questionable: #FFEB3B;   /* 2.0 < |z| < 3.0 */
  --pt-unsatisfactory: #D32F2F; /* |z| ≥ 3.0 */
}
```

### 3.2 Visual Effects

```css
:root {
  /* Shadows - Depth levels */
  --shadow-xs: 0 1px 2px rgba(0, 0, 0, 0.06);
  --shadow-sm: 0 2px 4px rgba(0, 0, 0, 0.08);
  --shadow-md: 0 4px 12px rgba(0, 0, 0, 0.10);
  --shadow-lg: 0 8px 24px rgba(0, 0, 0, 0.14);
  --shadow-focus: 0 0 0 3px rgba(253, 185, 19, 0.3);
  
  /* Border Radius */
  --radius-sm: 6px;   /* Small elements */
  --radius-md: 8px;   /* Buttons, inputs */
  --radius-lg: 12px;  /* Cards, panels */
  --radius-xl: 16px;  /* Large containers */
  
  /* Transitions */
  --transition-fast: 150ms ease;
  --transition-normal: 250ms ease;
  --transition-slow: 350ms cubic-bezier(0.4, 0, 0.2, 1);
}
```

### 3.3 Spacing Scale

```css
:root {
  --space-xs: 0.25rem;   /* 4px */
  --space-sm: 0.5rem;    /* 8px */
  --space-md: 1rem;      /* 16px */
  --space-lg: 1.5rem;    /* 24px */
  --space-xl: 2rem;      /* 32px */
  --space-xxl: 3rem;     /* 48px */
}
```

---

## 4. Component Styling

### 4.1 Base Styles

```css
body {
  background-color: var(--pt-bg);
  color: var(--pt-fg);
  font-family: 'Droid Sans', -apple-system, BlinkMacSystemFont, 
               'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;
  font-size: 0.9375rem;  /* 15px */
  line-height: 1.6;
  -webkit-font-smoothing: antialiased;
}

.container-fluid {
  max-width: 1600px;
  margin: 0 auto;
  padding: var(--space-lg) var(--space-xl);
}
```

### 4.2 Cards & Panels

Cards use a clean design with subtle shadows and hover effects:

```css
.well, .card, .panel {
  background-color: var(--pt-bg-card);
  border: 1px solid var(--pt-border);
  border-radius: var(--radius-lg);
  box-shadow: var(--shadow-sm);
  padding: var(--space-lg);
  transition: box-shadow var(--transition-normal);
}

.well:hover, .card:hover {
  box-shadow: var(--shadow-md);
}

.card-header {
  background: linear-gradient(135deg, 
    var(--pt-primary-subtle) 0%, 
    var(--pt-bg-card) 100%);
  border-bottom: 1px solid var(--pt-border);
}
```

### 4.3 Sidebar Navigation

The sidebar has a distinctive yellow accent border:

```css
.sidebar .well {
  background: linear-gradient(180deg, 
    var(--pt-bg-card) 0%, 
    var(--pt-primary-subtle) 100%);
  border-left: 4px solid var(--pt-primary);
  border-radius: 0 var(--radius-lg) var(--radius-lg) 0;
}
```

### 4.4 Navigation Tabs

Tabs feature an animated yellow underline for the active state:

```css
.nav-tabs .nav-link.active::after {
  content: '';
  position: absolute;
  bottom: -2px;
  left: 0;
  right: 0;
  height: 3px;
  background: var(--pt-primary);
  border-radius: 3px 3px 0 0;
}

.navlist-panel .nav-link.active {
  background: linear-gradient(90deg, 
    var(--pt-primary) 0%, 
    var(--pt-primary-light) 100%);
  color: var(--pt-secondary);
}
```

### 4.5 Form Controls

All form inputs have consistent styling with focus states:

```css
.form-control, input, select, textarea {
  background-color: var(--pt-bg-card);
  border: 1.5px solid var(--pt-border);
  border-radius: var(--radius-md);
  padding: var(--space-sm) var(--space-md);
  transition: border-color var(--transition-fast), 
              box-shadow var(--transition-fast);
}

.form-control:focus {
  border-color: var(--pt-primary);
  box-shadow: var(--shadow-focus);
  outline: none;
}
```

#### File Input (Drag & Drop Style)

```css
input[type="file"] {
  padding: var(--space-md);
  background-color: var(--pt-primary-subtle);
  border: 2px dashed var(--pt-primary);
  border-radius: var(--radius-md);
  cursor: pointer;
}

input[type="file"]:hover {
  background-color: var(--pt-primary-light);
  border-color: var(--pt-primary-dark);
}
```

#### Slider Input (IRS Plugin)

```css
.irs--shiny .irs-bar {
  background: var(--pt-primary);
  border-color: var(--pt-primary);
}

.irs--shiny .irs-handle {
  background: var(--pt-primary);
  border-color: var(--pt-primary-dark);
}
```

### 4.6 Buttons

Buttons use gradients for depth with hover lift effects:

```css
.btn, button, .action-button {
  font-weight: 500;
  padding: var(--space-sm) var(--space-lg);
  border-radius: var(--radius-md);
  transition: all var(--transition-fast);
}

.btn-primary, .btn-default {
  background: linear-gradient(135deg, 
    var(--pt-primary) 0%, 
    var(--pt-primary-dark) 100%);
  color: var(--pt-secondary);
  box-shadow: var(--shadow-sm);
}

.btn-primary:hover {
  background: linear-gradient(135deg, 
    var(--pt-primary-light) 0%, 
    var(--pt-primary) 100%);
  transform: translateY(-1px);
  box-shadow: var(--shadow-md);
}

.btn-primary:active {
  transform: translateY(0);
  box-shadow: var(--shadow-xs);
}
```

#### Button Variants

| Class | Color | Use Case |
|-------|-------|----------|
| `.btn-primary` | Yellow gradient | Primary actions |
| `.btn-success` | Green gradient | Confirmations |
| `.btn-danger` | Red gradient | Destructive actions |
| `.btn-info` | Blue gradient | Informational |
| `.btn-secondary` | Gray outline | Secondary actions |

### 4.7 Tables & DataTables

Tables have styled headers with yellow accent:

```css
.table thead th {
  background: linear-gradient(180deg, 
    var(--pt-primary-subtle) 0%, 
    var(--pt-bg-card) 100%);
  color: var(--pt-secondary);
  font-weight: 600;
  border-bottom: 2px solid var(--pt-primary);
  text-transform: uppercase;
  letter-spacing: 0.03em;
}

.table tbody tr:hover {
  background-color: var(--pt-primary-subtle);
}

/* Alternating rows */
.table tbody tr:nth-child(even) {
  background-color: rgba(253, 185, 19, 0.03);
}
```

#### DataTables Pagination

```css
.dataTables_paginate .paginate_button.current,
.dataTables_paginate .paginate_button:hover {
  background: var(--pt-primary) !important;
  color: var(--pt-secondary) !important;
}
```

### 4.8 Score Status Badges

These badges display PT score classifications per ISO 13528:

```css
/* Satisfactory: |z| ≤ 2.0 */
.badge-satisfactory, .status-satisfactory {
  background-color: var(--pt-satisfactory);  /* #00B050 */
  color: white;
  padding: var(--space-xs) var(--space-sm);
  border-radius: var(--radius-sm);
  font-weight: 500;
}

/* Questionable: 2.0 < |z| < 3.0 */
.badge-questionable, .status-questionable {
  background-color: var(--pt-questionable);  /* #FFEB3B */
  color: var(--pt-secondary);
}

/* Unsatisfactory: |z| ≥ 3.0 */
.badge-unsatisfactory, .status-unsatisfactory {
  background-color: var(--pt-unsatisfactory);  /* #D32F2F */
  color: white;
}
```

### 4.9 Alerts & Notifications

Shiny notifications use colored left borders:

```css
.alert-success, .shiny-notification-message {
  background-color: #D4EDDA;
  color: #155724;
  border-left: 4px solid var(--pt-success);
}

.alert-warning, .shiny-notification-warning {
  background-color: #FFF3CD;
  color: #856404;
  border-left: 4px solid var(--pt-warning);
}

.alert-danger, .shiny-notification-error {
  background-color: #F8D7DA;
  color: #721C24;
  border-left: 4px solid var(--pt-danger);
}
```

### 4.10 KPI/Value Box Cards

Special styling for metrics display:

```css
.value-box, .kpi-card {
  background: linear-gradient(135deg, 
    var(--pt-bg-card) 0%, 
    var(--pt-primary-subtle) 100%);
  border-radius: var(--radius-lg);
  padding: var(--space-lg);
  text-align: center;
  box-shadow: var(--shadow-md);
  border-left: 4px solid var(--pt-primary);
  transition: transform var(--transition-normal);
}

.value-box:hover {
  transform: translateY(-4px);
  box-shadow: var(--shadow-lg);
}

.value-box .value {
  font-size: 2.5rem;
  font-weight: 700;
  color: var(--pt-primary-dark);
}

.value-box .label {
  font-size: 0.875rem;
  color: var(--pt-fg-muted);
  text-transform: uppercase;
  letter-spacing: 0.05em;
}
```

### 4.11 Enhanced Header with Logo

The application features a prominent header with institutional branding:

```css
.app-header {
  background: linear-gradient(135deg, var(--pt-bg-card) 0%, var(--pt-primary-subtle) 100%);
  border-bottom: 4px solid var(--pt-primary);
  padding: var(--space-xl);
  margin-bottom: var(--space-xl);
  border-radius: var(--radius-lg);
  box-shadow: var(--shadow-md);
}

.header-content {
  display: flex;
  align-items: center;
  gap: var(--space-xl);
  max-width: 1600px;
  margin: 0 auto;
}

.logo-container {
  flex-shrink: 0;
}

.unal-logo {
  height: 100px;
  width: auto;
  object-fit: contain;
}

.title-container {
  flex-grow: 1;
}

.app-title {
  font-size: 1.75rem;
  font-weight: 700;
  color: var(--pt-secondary);
  margin-bottom: var(--space-sm);
}

.app-subtitle {
  font-size: 1.25rem;
  font-weight: 500;
  color: var(--pt-primary-dark);
}

.app-institution {
  font-size: 0.875rem;
  color: var(--pt-fg-muted);
}
```

#### Responsive Header

```css
@media (max-width: 768px) {
  .header-content {
    flex-direction: column;
    text-align: center;
  }
  .unal-logo {
    height: 60px;
  }
  .app-title {
    font-size: 1.25rem;
  }
}
```

### 4.12 shadcn-Inspired Card Component

A modern card component inspired by the shadcn/ui design system:

```css
.shadcn-card {
  background-color: var(--pt-bg-card);
  border: 1px solid var(--pt-border);
  border-radius: var(--radius-lg);
  box-shadow: var(--shadow-sm);
  overflow: hidden;
  transition: box-shadow var(--transition-normal);
  margin-bottom: var(--space-lg);
}

.shadcn-card:hover {
  box-shadow: var(--shadow-md);
}

.shadcn-card-header {
  background: linear-gradient(135deg, var(--pt-primary-subtle) 0%, transparent 100%);
  border-bottom: 1px solid var(--pt-border);
  padding: var(--space-md) var(--space-lg);
}

.shadcn-card-title {
  font-size: 1.125rem;
  font-weight: 600;
  color: var(--pt-secondary);
  margin: 0;
  display: flex;
  align-items: center;
  gap: var(--space-sm);
}

.shadcn-card-title i {
  color: var(--pt-primary);
}

.shadcn-card-description {
  font-size: 0.875rem;
  color: var(--pt-fg-muted);
  margin-top: var(--space-xs);
}

.shadcn-card-content {
  padding: var(--space-lg);
}

.shadcn-card-footer {
  border-top: 1px solid var(--pt-border);
  background-color: rgba(253, 185, 19, 0.02);
  padding: var(--space-md) var(--space-lg);
  display: flex;
  justify-content: flex-end;
  gap: var(--space-sm);
}
```

#### Usage in Shiny

```r
div(class = "shadcn-card",
  div(class = "shadcn-card-header",
    h3(class = "shadcn-card-title", 
       icon("upload"), " Upload Data Files"
    ),
    p(class = "shadcn-card-description",
      "Please upload the required CSV files for analysis."
    )
  ),
  div(class = "shadcn-card-content",
    # Content here
  )
)
```

### 4.13 shadcn-Inspired Alert Component

Modern alert styling with icon support:

```css
.alert-modern {
  border-radius: var(--radius-md);
  border: 1px solid;
  padding: var(--space-md);
  margin-bottom: var(--space-md);
  display: flex;
  gap: var(--space-md);
  align-items: flex-start;
}

.alert-modern-icon {
  flex-shrink: 0;
  width: 20px;
  height: 20px;
  margin-top: 2px;
}

.alert-modern-content {
  flex-grow: 1;
}

.alert-modern-title {
  font-weight: 600;
  margin-bottom: var(--space-xs);
  font-size: 0.9375rem;
}

.alert-modern-description {
  font-size: 0.875rem;
  opacity: 0.9;
  line-height: 1.5;
}
```

#### Alert Variants

| Class | Background | Border | Use Case |
|-------|------------|--------|----------|
| `.alert-modern.info` | #EBF8FF | #3182CE | Information messages |
| `.alert-modern.success` | #F0FFF4 | #38A169 | Success confirmations |
| `.alert-modern.warning` | #FFFAF0 | #DD6B20 | Warning notices |
| `.alert-modern.error` | #FFF5F5 | #E53E3E | Error messages |

### 4.14 shadcn-Inspired Badge Component

Modern badges for labels and status indicators:

```css
.badge-modern {
  display: inline-flex;
  align-items: center;
  gap: var(--space-xs);
  padding: var(--space-xs) var(--space-sm);
  border-radius: var(--radius-sm);
  font-size: 0.75rem;
  font-weight: 600;
  text-transform: uppercase;
  letter-spacing: 0.05em;
  border: 1px solid transparent;
  transition: all var(--transition-fast);
  line-height: 1;
}
```

#### Badge Variants

| Class | Style | Use Case |
|-------|-------|----------|
| `.badge-modern.default` | Yellow accent | Default labels |
| `.badge-modern.outline` | Transparent with border | Secondary labels |
| `.badge-modern.secondary` | Gray background | Subdued labels |
| `.badge-modern.satisfactory` | Green | PT score ≤ 2.0 |
| `.badge-modern.questionable` | Yellow | PT score 2.0-3.0 |
| `.badge-modern.unsatisfactory` | Red | PT score ≥ 3.0 |

### 4.15 Enhanced Upload Components

Grid-based layout for file upload areas:

```css
.upload-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));
  gap: var(--space-lg);
}

.upload-item {
  position: relative;
}

.upload-label {
  display: flex;
  align-items: center;
  gap: var(--space-sm);
  font-weight: 600;
  color: var(--pt-secondary);
  margin-bottom: var(--space-sm);
}

.upload-label i {
  color: var(--pt-primary);
  font-size: 1.25rem;
}
```

#### Enhanced File Input

```css
.shiny-input-container input[type="file"] {
  position: relative;
  padding: var(--space-lg);
  background: linear-gradient(135deg, var(--pt-primary-subtle) 0%, var(--pt-bg-card) 100%);
  border: 2px dashed var(--pt-primary);
  border-radius: var(--radius-md);
  cursor: pointer;
  transition: all var(--transition-normal);
  text-align: center;
}

.shiny-input-container input[type="file"]:hover {
  background: var(--pt-primary-light);
  border-color: var(--pt-primary-dark);
  transform: translateY(-2px);
  box-shadow: var(--shadow-md);
}

.shiny-input-container input[type="file"]::file-selector-button {
  padding: var(--space-sm) var(--space-lg);
  background: linear-gradient(135deg, var(--pt-primary) 0%, var(--pt-primary-dark) 100%);
  color: var(--pt-secondary);
  border: none;
  border-radius: var(--radius-sm);
  font-weight: 600;
  cursor: pointer;
}
```

### 4.16 Modern Footer

Three-column footer with institutional information:

```css
.app-footer-modern {
  background: #585858;
  color: white;
  padding: var(--space-lg) 0 var(--space-sm);
  margin-top: var(--space-lg);
  border-top: 4px solid var(--pt-primary);
}

.footer-content {
  max-width: 1600px;
  margin: 0 auto;
  padding: 0 var(--space-xl);
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));
  gap: var(--space-xl);
  margin-bottom: var(--space-md);
}

.footer-section h4 {
  color: var(--pt-primary);
  font-size: 1rem;
  font-weight: 600;
  margin-bottom: var(--space-xs);
  text-transform: uppercase;
  letter-spacing: 0.05em;
}

.footer-section p {
  color: rgba(255, 255, 255, 0.8);
  font-size: 0.875rem;
  margin-bottom: var(--space-xs);
  line-height: 1.4;
}

.footer-section a {
  color: var(--pt-primary-light);
  text-decoration: none;
  transition: color var(--transition-fast);
}

.footer-section a:hover {
  color: var(--pt-primary);
  text-decoration: underline;
}

.footer-bottom {
  text-align: center;
  padding-top: var(--space-sm);
  border-top: 1px solid rgba(255, 255, 255, 0.1);
}

.footer-bottom p {
  color: rgba(255, 255, 255, 0.6);
  font-size: 0.8125rem;
}
```

#### Footer Structure in Shiny

```r
tags$footer(class = "app-footer-modern",
  div(class = "footer-content",
    div(class = "footer-section",
      h4("Proyecto"),
      p(em("Descripción del proyecto..."))
    ),
    div(class = "footer-section",
      h4("Instituciones"),
      p("Laboratorio CALAIRE"),
      p("Universidad Nacional de Colombia")
    ),
    div(class = "footer-section",
      h4("Contacto"),
      p(tags$a(href = "mailto:email@example.com", "email@example.com"))
    )
  ),
  div(class = "footer-bottom",
    p("© 2026 Universidad Nacional de Colombia.")
  )
)
```

---

## 5. Responsive Design

### 5.1 Mobile Breakpoint (≤768px)

```css
@media (max-width: 768px) {
  .container-fluid {
    padding: var(--space-md);
  }
  
  .well, .card {
    padding: var(--space-md);
  }
  
  .nav-tabs .nav-link {
    padding: var(--space-xs) var(--space-md);
    font-size: 0.875rem;
  }
  
  .table thead th,
  .table tbody td {
    padding: var(--space-sm);
    font-size: 0.875rem;
  }
}
```

---

## 6. Print Styles

For generating printable reports:

```css
@media print {
  body {
    background: white;
  }
  
  /* Hide interactive elements */
  .btn, .nav-tabs, .dataTables_filter,
  .dataTables_length, .dataTables_paginate {
    display: none !important;
  }
  
  /* Remove shadows for cleaner printing */
  .well, .card, .table {
    box-shadow: none;
    border: 1px solid #ddd;
  }
}
```

---

## 7. Accessibility Features

### 7.1 Focus Indicators

```css
:focus-visible {
  outline: 2px solid var(--pt-primary);
  outline-offset: 2px;
}
```

### 7.2 Screen Reader Support

```css
.sr-only {
  position: absolute;
  width: 1px;
  height: 1px;
  padding: 0;
  margin: -1px;
  overflow: hidden;
  clip: rect(0, 0, 0, 0);
  white-space: nowrap;
  border: 0;
}
```

### 7.3 High Contrast Mode

```css
@media (prefers-contrast: high) {
  :root {
    --pt-border: #333;
    --shadow-sm: none;
    --shadow-md: none;
    --shadow-lg: none;
  }
  
  .btn, .form-control, .well, .card {
    border: 2px solid currentColor;
  }
}
```

### 7.4 Reduced Motion

```css
@media (prefers-reduced-motion: reduce) {
  *, *::before, *::after {
    animation-duration: 0.01ms !important;
    animation-iteration-count: 1 !important;
    transition-duration: 0.01ms !important;
  }
}
```

---

## 8. Custom Scrollbars

### 8.1 WebKit Browsers (Chrome, Safari, Edge)

```css
::-webkit-scrollbar {
  width: 10px;
  height: 10px;
}

::-webkit-scrollbar-track {
  background: var(--pt-bg);
  border-radius: var(--radius-sm);
}

::-webkit-scrollbar-thumb {
  background: linear-gradient(180deg, 
    var(--pt-primary-light) 0%, 
    var(--pt-primary) 100%);
  border-radius: var(--radius-sm);
  border: 2px solid var(--pt-bg);
}

::-webkit-scrollbar-thumb:hover {
  background: var(--pt-primary-dark);
}
```

### 8.2 Firefox

```css
* {
  scrollbar-width: thin;
  scrollbar-color: var(--pt-primary) var(--pt-bg);
}
```

---

## 9. Third-Party Component Styling

### 9.1 RHandsontable (Spreadsheet)

```css
.handsontable {
  font-family: inherit;
}

.handsontable th {
  background: var(--pt-primary-subtle) !important;
  color: var(--pt-secondary) !important;
  font-weight: 600;
}

.handsontable th, .handsontable td {
  border-color: var(--pt-border) !important;
}

/* Selected cell highlight */
.handsontable td.current {
  background-color: rgba(253, 185, 19, 0.15) !important;
}

.handsontable td.area {
  background-color: rgba(253, 185, 19, 0.1) !important;
}
```

### 9.2 Selectize Dropdowns

```css
.selectize-dropdown {
  border: 1px solid var(--pt-border);
  border-radius: var(--radius-md);
  box-shadow: var(--shadow-md);
}

.selectize-dropdown .option:hover,
.selectize-dropdown .option.active {
  background-color: var(--pt-primary-subtle);
}

.selectize-dropdown .option.selected {
  background-color: var(--pt-primary);
  color: var(--pt-secondary);
}
```

### 9.3 Plotly Containers

```css
.shiny-plot-output, .plotly {
  background-color: var(--pt-bg-card);
  border-radius: var(--radius-lg);
  padding: var(--space-md);
  box-shadow: var(--shadow-sm);
  margin-bottom: var(--space-lg);
}
```

---

## 10. Component Visual Reference

### Color Palette Diagram

```mermaid
graph LR
    subgraph Primary["Primary Palette"]
        P1["#FDB913\n--pt-primary"]
        P2["#FFD54F\n--pt-primary-light"]
        P3["#E5A610\n--pt-primary-dark"]
        P4["#F5F5F0\n--pt-primary-subtle"]
    end
    
    subgraph Neutral["Neutral Palette"]
        N1["#E8EAED\n--pt-bg"]
        N2["#F5F6F7\n--pt-bg-card"]
        N3["#1F2937\n--pt-fg"]
        N4["#6B7280\n--pt-fg-muted"]
    end
    
    subgraph Semantic["Semantic Colors"]
        S1["#38A169\n--pt-success"]
        S2["#ECC94B\n--pt-warning"]
        S3["#E53E3E\n--pt-danger"]
        S4["#3182CE\n--pt-info"]
    end
    
    subgraph Scores["Score Colors"]
        SC1["#00B050\nSatisfactory"]
        SC2["#FFEB3B\nQuestionable"]
        SC3["#D32F2F\nUnsatisfactory"]
    end
```

### Component Hierarchy

```mermaid
flowchart TD
    ROOT[":root CSS Variables"] --> BASE["Base Styles\nbody, container"]
    BASE --> LAYOUT["Layout Components\ncards, sidebar, tabs"]
    LAYOUT --> FORMS["Form Controls\ninputs, buttons, sliders"]
    LAYOUT --> DATA["Data Display\ntables, plots, badges"]
    DATA --> FEEDBACK["Feedback\nalerts, progress, modals"]
    
    ROOT --> SHADCN["shadcn Components\ncards, alerts, badges"]
    ROOT --> HEADER["Header & Footer\nbranding, layout"]
    ROOT --> RESP["Responsive\n@media queries"]
    ROOT --> A11Y["Accessibility\ncontrast, motion, focus"]
    ROOT --> PRINT["Print Styles\n@media print"]
```

---

## 11. Customization Guide

### 11.1 Changing the Primary Color

To change the brand color from yellow to another color:

```css
:root {
  /* Example: Blue theme */
  --pt-primary: #3182CE;
  --pt-primary-light: #63B3ED;
  --pt-primary-dark: #2B6CB0;
  --pt-primary-subtle: #EBF8FF;
  --pt-border-focus: #3182CE;
}
```

### 11.2 Adding New Score Categories

```css
/* Example: Adding a "Warning" level */
.badge-warning-level, .status-warning-level {
  background-color: #FF9800;
  color: white;
  padding: var(--space-xs) var(--space-sm);
  border-radius: var(--radius-sm);
  font-weight: 500;
}
```

### 11.3 Creating Custom Value Boxes

```css
/* Success value box variant */
.value-box.success {
  border-left-color: var(--pt-success);
}

.value-box.success .value {
  color: var(--pt-success);
}
```

---

## 12. Troubleshooting

| Issue | Cause | Solution |
|-------|-------|----------|
| Styles not applying | CSS not loaded | Verify `www/appR.css` exists and path is correct |
| Variables undefined | Old browser | Use fallback values: `color: #FDB913; color: var(--pt-primary);` |
| Conflicts with bslib | Specificity issues | Add `!important` or increase selector specificity |
| Print layout broken | Interactive elements | Check `@media print` rules hide buttons/filters |
| Scrollbar not styled | Firefox | Firefox uses `scrollbar-color` property |

---

## 13. Performance Considerations

1. **Transitions**: Limited to fast properties (`transform`, `opacity`, `box-shadow`)
2. **Shadows**: Use CSS variables for consistent shadow levels
3. **Gradients**: Applied to static elements, not animated
4. **Hover Effects**: Subtle transforms (`translateY(-1px)`) instead of expensive reflows

---

## 14. See Also

- [15_architecture.md](15_architecture.md) - System architecture overview
- [16_customization.md](16_customization.md) - Theme and layout customization
- [bslib Documentation](https://rstudio.github.io/bslib/) - Bootstrap integration for Shiny
