...
## 1. Generalidades
Funciones para calcular y evaluar los puntajes de desempeño de los participantes ($z$, $z'$, $\zeta$, $E_n$) de acuerdo con la norma ISO 13528:2022 Sección 10. Estos puntajes cuantifican qué tan bien se alinea el resultado de un participante con el valor asignado y permiten la evaluación de la competencia del laboratorio en esquemas de ensayos de aptitud (PT).

La selección del puntaje depende de:
1.  Información disponible (valor asignado, $\sigma_{pt}$, incertidumbres).
2.  Propósito (tamizaje vs. evaluación detallada).
...
flowchart TD
    ...
    B -- No --> C{¿Es u_xpt significativa<br/>respecto a sigma_pt?}
    C -- No --> D[Usar puntaje z]
    C -- Yes --> E[Usar puntaje z']
    B -- Yes --> F{¿Evaluar incertidumbre<br/>reportada?}
    F -- No --> G[Usar z o z']
    F -- Yes --> H{¿Está disponible la<br/>incertidumbre expandida U?}
    H -- No --> I[Usar puntaje zeta]
    H -- Yes --> J[Usar puntaje En]
...
```mermaid
flowchart TD
    START[Calcular puntaje de PT] --> UNC_X{¿Incertidumbre del<br/>participante conocida?}
    
    UNC_X -- Yes --> UNC_XPT{¿Incertidumbre del valor<br/>asignado conocida?}
    UNC_XPT -- Yes --> ZETA[Usar puntaje zeta<br/>ζ = (x - x_pt) / sqrt(u_x² + u_xpt²)]
    UNC_XPT -- No --> ZPRIME[Usar puntaje z'<br/>z' = (x - x_pt) / sqrt(σ_pt² + u_xpt²)]
    
    UNC_X -- No --> SIGMA_PT{¿σ_pt especificada?}
    SIGMA_PT -- Yes --> Z[Usar puntaje z<br/>z = (x - x_pt) / σ_pt]
    SIGMA_PT -- No --> CALIBRATION{¿Comparación de<br/>calibración?}
    
    CALIBRATION -- Yes --> EN[Usar puntaje En<br/>En = (x - x_pt) / sqrt(U_x² + U_xpt²)]
...
| **ζ (zeta)** | $(x - x_{pt}) / \sqrt{u_x^2 + u_{xpt}^2}$ | Sí | $u_x$ (estándar) | Evaluar incertidumbre estándar reportada. |
| **En** | $(x - x_{pt}) / \sqrt{U_x^2 + U_{xpt}^2}$ | Sí | $U_x$ (expandida) | Compatibilidad metrológica (k=2). |
...
Métrica de desempeño estándar. Se usa cuando la incertidumbre del valor asignado es despreciable ($u(x_{pt}) \le 0.3 \sigma_{pt}$).
...
**Devuelve:** Valor del puntaje z (o `NA_real_` si `sigma_pt` ≤ 0).
...
Se usa cuando la incertidumbre del valor asignado es significativa ($u(x_{pt}) > 0.3 \sigma_{pt}$). Amplía los límites de aceptación para tener en cuenta esta incertidumbre.
...
### 5.1 Clasificación de puntajes z, z' y ζ
...
## 6. Propagación de Incertidumbre ($u_{xpt,def}$)

La incertidumbre estándar definitiva del valor asignado ($u_{xpt,def}$) combina múltiples fuentes, incluyendo caracterización, homogeneidad y estabilidad.
...
Una $u_{xpt}$ más alta conduce a valores de puntaje absolutos más bajos (evaluación más indulgente), ya que aumenta el denominador.
...
La función `classify_with_en()` combina los puntajes de desempeño (z/z') con $E_n$ para una evaluación más integral del desempeño del laboratorio.
...
| **a6** | No satisfactorio pero MU cubre | $\ge 3$ | $\le 1$ | - | Resultado deficiente, pero cubierto por una MU muy grande. |
...
| Código | Color Hexadecimal | Descripción |
| :--- | :--- | :--- |
...
| `mu_missing_z` | `#90A4AE` | Gris-Azul (MU faltante - z) |
...
### Escenario 3: Resultado Deficiente cubierto por una Incertidumbre Grande
...
## 9. Flujo de Trabajo en la Aplicación
...
- ~95% de los resultados deben caer dentro de $\pm 2$.
- ~99.7% de los resultados deben caer dentro de $\pm 3$.

### 10.2 Análisis de Patrones
| Patrón Observado | Posible Causa |
| :--- | :--- |
| Media $z \neq 0$ | Sesgo sistemático en $x_{pt}$ o en el grupo de participantes. |
| $DE(z) >> 1$ | $\sigma_{pt}$ está subestimada (los límites son demasiado estrictos). |
| $DE(z) << 1$ | $\sigma_{pt}$ está sobreestimada (los límites son demasiado holgados). |
...
- [03_pt_robust_stats.md](03_pt_robust_stats.md) - Media robusta y DE para valores asignados.
- [04_pt_homogeneity.md](04_pt_homogeneity.md) - Contribuciones de homogeneidad y estabilidad a $u_{xpt}$.
- [09_puntajes_pt.md](09_puntajes_pt.md) - Interfaz de usuario para puntajes de desempeño.
- [10_informe_global.md](10_informe_global.md) - Visualizaciones de mapas de calor de resultados.
