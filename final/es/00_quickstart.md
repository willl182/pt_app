# Guía de Inicio Rápido: Aplicación de Análisis de Datos de Ensayos de Aptitud (PT)

Esta guía le permitirá ejecutar la Aplicación de Ensayos de Aptitud en menos de 5 minutos. Esta aplicación implementa un conjunto de herramientas completo para el análisis de PT de gases contaminantes criterio (CO, SO2, NO, NO2, O3) siguiendo los estándares **ISO 17043:2024** e **ISO 13528:2022**.
...
### Paso 1: Instalar Paquetes de R Requeridos
...
La aplicación depende del paquete local `ptcalc` para los cálculos de ISO 13528/17043.
...
# Opción 2: Usando ejecución de R
R -e "shiny::runApp('cloned_app.R')"
```
*Nota: La aplicación será típicamente accesible en su navegador en `http://127.0.0.1:3838` o en el puerto mostrado en la consola.*
...
La aplicación requiere formatos CSV específicos. Se incluyen archivos de datos de muestra en la carpeta `data/`:

- `homogeneity.csv`: Datos de la prueba de homogeneidad
- `stability.csv`: Datos de la prueba de estabilidad
- `summary_n4.csv`: Escenario de 4 participantes
...
#### 2. Archivos de Resumen de Participantes (`summary_n*.csv`)
```csv
...
1. Diríjase a la pestaña **"Carga de datos"**.
...
5. Use la pestaña **"Estabilidad"** para comparar los resultados de homogeneidad vs. estabilidad.

### Paso 4: Calcular Valor Asignado
1. Diríjase a la pestaña **"Valor Asignado"**.
...
3. Haga clic en **"Calcular puntajes"** para ver las métricas de desempeño para todos los laboratorios.

### Paso 6: Generar Informes
1. Diríjase a la pestaña **"Generación de Informes"**.
...
Asegúrese de que sus archivos CSV contengan todas las columnas requeridas con los nombres exactos (distingue mayúsculas de minúsculas).
...
- [API del Paquete](../02_ptcalc_package.md) - Referencia de funciones matemáticas
