...
| **Plantilla Principal** | `reports/report_template.Rmd` |
...
1. **Configuración**: El usuario ajusta los parámetros en la interfaz (UI).
2. **Preparación**: Al hacer clic en descargar, se crea un directorio temporal.
3. **Copiado**: Se copia la plantilla `report_template.Rmd` (y otros archivos necesarios) al directorio temporal para evitar conflictos de escritura.
...
## 5. Integración con RMarkdown (Plantilla)

El archivo `reports/report_template.Rmd` es el núcleo del informe. Está diseñado para ser totalmente paramétrico.
...
| **HTML** | Interactividad, gráficos dinámicos (plotly), visualización rápida en navegadores. | Revisión interna o paneles de control (dashboards) de resultados. |
...
La interfaz permite ingresar los nombres para el bloque de firmas final:
*   Coordinador EA.
...
| "Puntajes no calculados" | Se intenta generar informe antes de ejecutar el cálculo. | Ir a la pestaña **PT Scores** (Puntajes PT) y hacer clic en "Calcular Puntajes". |
| Error al renderizar Word | La plantilla `report_template.Rmd` está abierta o corrupta. | Verificar integridad del archivo en la carpeta `reports/`. |
...
*   [09_puntajes_pt.md](09_puntajes_pt.md): Lógica de cálculo de puntajes.
*   [13_valores_atipicos.md](13_valores_atipicos.md): Detalles del test de Grubbs.
*   [Glosario.md](Glosario.md): Definiciones de términos estadísticos.
