...
fluidRow(
  ...
  purrr::iwalk(global_combo_specs, function(spec, combo_key) {
    ...
    render_global_score_heatmap(
      ...
      combo_key, "z_score", "z_score_eval", score_heatmap_palettes$z, "Mapa de calor z-score"
    )

    render_global_score_heatmap(
      paste0("global_heatmap_zeta_", combo_key), 
      combo_key, "zeta_score", "zeta_score_eval", score_heatmap_palettes$zeta, "Mapa de calor zeta-score"
    )

    render_global_score_heatmap(
      paste0("global_heatmap_en_", combo_key), 
      combo_key, "En_score", "En_score_eval", score_heatmap_palettes$en, "Mapa de calor En-score"
    )

    render_global_classification_heatmap(
      paste0("global_class_heatmap_z_", combo_key), 
      combo_key, "classification_z_en_code", "classification_z_en", "Clasificación z + En"
    )

    render_global_classification_heatmap(
      paste0("global_class_heatmap_zprime_", combo_key), 
      combo_key, "classification_zprime_en_code", "classification_zprime_en", "Clasificación z' + En"
    )
  })
  ...
  for (key in names(score_combo_info)) {
    local({
      ...
      output[[paste0("z_plot_", combo_key)]] <- renderPlotly({
        ...
        p <- plot_scores(combo$data, "z_score", combo$title, "Límites de advertencia |z|=2, acción |z|=3", "Z-Score", warn_limits = c(-2, 2), action_limits = c(-3, 3))
        ggplotly(p)
      })
      ...
      output[[paste0("zprime_plot_", combo_key)]] <- renderPlotly({
        ...
        combo <- res$combos[[combo_key]]
        if (is.null(combo) || !is.null(combo$error)) return(NULL)
        p <- plot_scores(combo$data, "z_prime_score", combo$title, "Límites de advertencia |z'|=2, acción |z'|=3", "Z'-Score", warn_limits = c(-2, 2), action_limits = c(-3, 3))
        ggplotly(p)
      })
      ...
      output[[paste0("zeta_plot_", combo_key)]] <- renderPlotly({
        ...
        combo <- res$combos[[combo_key]]
        if (is.null(combo) || !is.null(combo$error)) return(NULL)
        p <- plot_scores(combo$data, "zeta_score", combo$title, "Límites de advertencia |ζ|=2, acción |ζ|=3", "Zeta-Score", warn_limits = c(-2, 2), action_limits = c(-3, 3))
        ggplotly(p)
      })
      ...
      output[[paste0("en_plot_", combo_key)]] <- renderPlotly({
        ...
        combo <- res$combos[[combo_key]]
        if (is.null(combo) || !is.null(combo$error)) return(NULL)
        p <- plot_scores(combo$data, "En_score", combo$title, "Límite de acción |En|=1", "En-Score", action_limits = c(-1, 1))
        ggplotly(p)
      })
    })
  }
  ...
  output$download_report <- downloadHandler(
    filename = function() {
      ext <- switch(input$report_format, html = "html", pdf = "pdf", word = "docx")
