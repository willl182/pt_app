# Core statistical functions for Proficiency Testing Analysis

#' Calculate Normalized Interquartile Range (nIQR)
#'
#' @param x Numeric vector
#' @return Numeric value of nIQR or NA if insufficient data
...
#' Run Algorithm A (ISO 13528:2022)
#'
#' Iteratively calculates robust mean and standard deviation.
#'
#' @param values Numeric vector of values
#' @param ids Vector of participant IDs
#' @param max_iter Maximum number of iterations
#' @return List containing assigned_value, robust_sd, iterations (df), weights (df), converged (bool), effective_weight, and error (or NULL)
run_algorithm_a <- function(values, ids, max_iter = 50) {
  ...
  iteration_df <- if (length(iteration_records) > 0) {
    dplyr::bind_rows(iteration_records)
  } else {
    tibble::tibble()
  }
...
#' Compute Consensus Metrics (Median, MADe, nIQR)
#'
#' @param values Numeric vector of values
#' @return List of results
compute_consensus_metrics <- function(values) {
  ...
  list(
    x_pt_median = x_pt2,
    mad_val = mad_val,
    sigma_pt_2a = sigma_pt_2a,
    sigma_pt_2b = sigma_pt_2b,
