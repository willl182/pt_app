...
def load_summary_combo(filepath, pollutant, level, exclude_ref=True):
    """Alias de compatibilidad para cargar summary por combo."""
    return load_summary_data(filepath, pollutant, level, exclude_ref=exclude_ref)
...
def mad_e(values):
    """MADe = 1.483 * MAD, usando mediana de desviaciones absolutas."""
    finite_vals = [v for v in values if math.isfinite(v)]
    if not finite_vals:
        return float("nan")
    med = median(finite_vals)
    abs_dev = [abs(v - med) for v in finite_vals]
    mad = median(abs_dev)
    return 1.483 * mad

def niqr(values):
    """nIQR = 0.7413 * IQR con cuartiles tipo 7."""
    iqr = iqr_type7(values)
...
def canonical_row(
    ...
    """Construir una fila canónica con diferencias y estado."""
    comparison = compare_values(app_value, r_value, python_value, tol=tolerance)
    return {
        "combo_id": combo_id,
        "pollutant": pollutant,
        "level": level,
        ...
        "app_value": app_value,
        "diff_r_python": comparison["diff_r_python"],
        "diff_app_r": comparison["diff_app_r"],
        "diff_app_python": comparison["diff_app_python"],
