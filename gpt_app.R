...
")
  ...
  plot_scores <- function(df, score_col, title, subtitle, ylab, warn_limits = NULL, action_limits = NULL) {
    ...
    if (all(!is.finite(score_values))) {
      ...
      return(plotly::ggplotly(empty_plot, tooltip = NULL))
    }
  ...
  render_global_score_heatmap <- function(output_id, combo_key, score_col, eval_col, palette, title_prefix) {
    output[[output_id]] <- renderPlotly({
      ...
      p <- ggplot(plot_data, aes(x = run_label, y = participant_id, fill = evaluation, text = tile_label)) +
      ...
      plotly::ggplotly(p, tooltip = c("x", "y", "text", "fill"))
    })
  }
  ...
  render_global_classification_heatmap <- function(output_id, combo_key, code_col, label_col, title_prefix) {
    output[[output_id]] <- renderPlotly({
      ...
      p <- ggplot(plot_data, aes(x = run_label, y = participant_id, fill = fill_code, text = class_label)) +
        geom_tile(color = "white") +
  ...
  output$scores_participant_tabs <- renderUI({
    ...
    tab_panels <- lapply(participants, function(pid) {
      ...
      output[[plot_id]] <- renderPlotly({
        ...
        p_values_pl <- plotly::ggplotly(p_values, tooltip = c("x", "y", "colour"))
        p_z_pl <- plotly::ggplotly(p_z, tooltip = c("x", "y", "colour"))
        p_zeta_pl <- plotly::ggplotly(p_zeta, tooltip = c("x", "y", "colour"))
        p_en_pl <- plotly::ggplotly(p_en, tooltip = c("x", "y", "colour"))

        p_z_pl <- plotly::layout(p_z_pl, showlegend = TRUE)
        p_zeta_pl <- plotly::layout(p_zeta_pl, showlegend = FALSE)
        p_en_pl <- plotly::layout(p_en_pl, showlegend = FALSE)
