...
# Configurar directorio de trabajo dinámicamente
get_project_root <- function() {
  ...
  if (basename(current_wd) == "06_app_logica") {
    return(dirname(current_wd))
  }
  
  if (basename(current_wd) == "Entregables_pt_app") {
    return(dirname(current_wd))
  }
...
source("Entregables_pt_app/06_app_logica/app_v06.R")
...
# Guardar CSV
setwd("Entregables_pt_app/06_app_logica/tests")
write.csv(results_df, "test_06_logica.csv", row.names = FALSE)
cat("CSV guardado en: Entregables_pt_app/06_app_logica/tests/test_06_logica.csv\n")
