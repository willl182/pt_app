...
calculate_stability_stats <- function(stab_data, hom_general_mean_homog, hom_stab_x_pt, hom_stab_sigma_pt) {
  if (is.vector(stab_data)) {
    if (length(stab_data) %% 2 == 0) {
      stab_data <- matrix(stab_data, ncol = 2, byrow = TRUE)
    } else {
      stab_data <- matrix(stab_data, ncol = 1)
    }
  }
  ...
  list(
    ...
    hom_mean = hom_general_mean_homog,
    error = NULL
  )
}
