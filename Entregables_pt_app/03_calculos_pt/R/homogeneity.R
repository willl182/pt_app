...
calcular_criterio_expandido_homogeneidad <- function(sigma_pt, sw, g = 10) {
  if (missing(g)) {
    g <- 10
  }
  # c_exp = sqrt(F1*(0.3*sigma_pt)^2 + F2*(sw)^2)
  ...
  f1 <- f_table$f1[idx]
  f2 <- f_table$f2[idx]
  sqrt(f1 * (0.3 * sigma_pt)^2 + f2 * sw^2)
}
...
analizar_homogeneidad <- function(datos_homogeneidad, contaminante, nivel, sigma_pt) {
  ...
  c_expanded <- calcular_criterio_expandido_homogeneidad(stats$sigma_pt, stats$sw, stats$g)
